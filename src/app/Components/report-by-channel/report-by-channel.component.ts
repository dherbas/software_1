...
export class ReportByChannelComponent implements OnInit {
  ...
  init(): void {
    this.form = this.formBuilder.group({
      type_search: [EnumTypeSearchReports.Vale.toString(), [Validators.required]],
  ...
  reportByChannel() {
    ...
    this.service.getReportByChannel(serviceCode, this.StartDate.value, this.EndDate.value, this.form.get('type_search').value)
      .subscribe((resultado) => {
          this.data = [];
  ...
  downloadXlsx(): void {
    const filename =
      'Canal por producto -' + moment().format('DDMMYYYY');
    this.blockUI.start();
    const serviceCode = this.user.list_code_service?.length > 1 ? this.form.get('service_code').value : this.user.list_code_service[0]?.code;

    this.service.getReportByChannelXlsx(serviceCode, this.StartDate.value, this.EndDate.value, this.form.get('type_search').value)
      .subscribe(
        (response) => {
