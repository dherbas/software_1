...
export class PosConfigurationComponent implements OnInit {
  ...
  edit(i: number) {
    ...
    const dialogRef = this.dialog.open(PosConfigurationDialogComponent, this.dialogConfig);

    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.configuration[i] = result;
        this.extraData.pos_configuration = this.configuration;
        this.userService.updateExtraData(this.userSession.id, this.extraData).subscribe(
          (resultado) => {
